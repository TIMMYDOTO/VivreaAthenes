...
- (void)setState:(SVInfiniteScrollingState)newState {
  ...
  //  NSLog(@"%lu",(unsigned long)newState);
    if(_state == newState)
        return;
    ...
    if(hasCustomView) {
        ...
        CGPoint origin = CGPointMake(roundf((self.bounds.size.width-viewBounds.size.width)/2), roundf((self.bounds.size.height-viewBounds.size.height)/2));
        [self.activityIndicatorView setFrame:CGRectMake(origin.x, origin.y - viewBounds.size.height/2, viewBounds.size.width, viewBounds.size.height)];
